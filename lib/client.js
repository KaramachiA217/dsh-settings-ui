    ...
    const plugin = {
      ...
      apply(ctx) {
        ...
        const service = {
           ...
           *   header?: { title, desc?, meta? }   // kit-rendered card head
           *   fields?: RowField[]                // kit Rows form (recommended)
           *   content?: (ctx) => ReactElement    // free content exit (fully custom)
           *   showIn?: 'official-tab' | 'settings-page' | 'both'  // default official-tab
           *   chrome?: 'full' | 'minimal'        // 'minimal' drops the kit shell
          ...
          pluginCard(config) {
            ...
            const chrome = config.chrome ?? 'full'
            ...
            // 卡体：kit 渲染外部壳（卡头/内容区/状态条），内容区 = fields Rows
            // 或 content 自由出口。content/fields 由服务/子槽注入，不 import
            // 官方卡 chrome（对齐束之纯度门）。
            const renderCardBody = (cfg, props) => {
            ...
            const renderCard = (props) => h(ErrorBoundary, { title: '该设置卡渲染出错' },
              h('div', { className: 'sui-root' }, renderCardBody(config, props)),
            )
