...
- **设置页右上角出现一个绿色圆点、被面板圆角裁掉**：样本页 A8 组演示 `.sui-fab` 时用内联 `position:static` 覆盖掉 `.sui-fab` 自身的 `position:fixed`（本意是让演示件留在文档流里）。但 `.sui-fab-badge` 是 `position:absolute; top:-3px; right:-3px` 的绿色圆点——父元素一旦变成 `static` 就**不再是包含块**，badge 改为相对**更外层**的定位祖先定位，于是飞到设置页右上角、紧贴「打开配置文件」按钮并被裁切。改为 `position:relative`：同样不脱离文档流，但保留包含块资格，badge 回到 FAB 自己的右上角。新增测试 56 锁定该契约，已做反向验证（退回 `static` 后测试 56 立即失败）。
...
- **`.sui-spec-token` 加 `padding-left:4px`**：让令牌体检行首的绿点（`.sui-spec-flag`）离 `.sui-spec-samples` 的 12px 圆角内壁留点空隙。
  - ⚠️ **更正**：本条最初被误记为「修复用户反馈的『右上角绿色圆点被裁切』」——**认错对象了**。用户指的是 `.sui-fab-badge`（见上一条 Fixed），不是这里的 `.sui-spec-flag`。该改动本身无害（行首留白），予以保留，但归因以本条为准。
